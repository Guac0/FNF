...
[{!isNull (findDisplay 46)}, {call phx_fnc_afkCheck}] call CBA_fnc_waitUntilAndExecute;
...
phx_safeZone_fnc_moveOutside = {
  ...
  if (phx_startGoodPos select 2 > 0.2) then {
    _object setpos phx_startGoodPos;
  } else {
    _object setpos (phx_startGoodPos findEmptyPosition [0, 50, typeOf _object]);
  };
