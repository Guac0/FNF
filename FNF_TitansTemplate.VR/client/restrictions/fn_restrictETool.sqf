...
phx_restrictETool_PFH = [{
  if (player getVariable ["ace_trenches_isplacing",false]) then {
    ...
    if (isNil "phx_restrictEToolAction") then {
      phx_restrictEToolAction = player addAction ["", {format ["Cannot place trench within %1m of an objective or within 6m of a road", phx_trenchObjDist] call phx_fnc_hintThenClear}, nil, 0, false, true, "defaultAction", "
        phx_etool_nearObjs || (count (ace_trenches_trench nearRoads 6) > 0) || (isOnRoad ace_trenches_trench)
      "];
    };
  } else {
    if (!isNil "phx_restrictEToolAction") then {
